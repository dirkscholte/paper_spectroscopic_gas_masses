...
\textcolor{magenta}{In the inference procedure using the SDSS MPA-JHU emission line flux measurements we multiply the formal emission line flux errors by the recommended multiplication factors\footnote{https://wwwmpa.mpa-garching.mpg.de/SDSS/DR7/raw\_data.html} to account for additional uncertainties due to continuum subtraction. Our signal-to-noise selections are made using the formal flux errors.}
...
In Figure \ref{fig:MH2_FH2_SFE_comparison_xCOLDGASS}, we show the distribution of the molecular gas masses, gas fractions ($f_{H2} \equiv M_{H2}/M_{\ast}$) and star formation efficiencies ($SFE \equiv SFR / M_{H2}$) we derive for the SDSS sample across the $M_{*}$-SFR plane. Overplotted are the xCOLD GASS galaxies with the same measurements inferred from direct CO(1-0) observations.  The figure shows that we are able to recover gas measurements for galaxies with a wide range of stellar masses and star formation rates, with the method performing well for star-forming main sequence galaxies and massive passive objects. 
